...
public class MachineProvisioningLocationFlagsSupplier extends AbstractLocationFlagsSupplier {
    ...
    /** returns the ports that the entity wants to use;
    ...
    public Collection<Integer> getRequiredOpenPorts() {
        ...
        Map<ConfigKey<?>, ?> allConfig = ((ConfigurationSupportInternal)entity().config()).getBag().getAllConfigAsConfigKeyMap();
        ...
        for (ConfigKey<?> k: configKeys) {
            ...
            if (PortRange.class.isAssignableFrom(k.getType()) || k.getName().matches(".*\\.port")) {
                value = ((ConfigurationSupportInternal)entity().config()).get(k);
            } else {
                ...
                Maybe<Object> maybeValue = ((ConfigurationSupportInternal)entity().config()).getRaw(k);
